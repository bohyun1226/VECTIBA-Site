# vectiba.com — 현재 상태 (2026-07-14 최신)
...
## 1-1. ⚠️ vectiba.com 은 마케팅 페이지 전용 — 실서비스 아님 (2026-07-14 확정)
실제 제품(셀러 AI 인테이크·매물 게시·경매·딜러 대시보드·포워더 대시보드)은 **별도 레포 `vectiba-app`**, 배포는 **`app.vectiba.com`**(포워더는 `partner.vectiba.com`), Vercel + Supabase("Vectiba-app" 프로젝트, `vectiba-app`쪽 `supabase/migrations/`가 진짜 스키마 — `listings`/`listing_media`/`bids`/`auctions` 등). DNS A레코드(`76.76.21.21`)만 대표님 액션 남으면 정식 도메인으로 뜸 — 그전엔 `vectiba-app.vercel.app`로 접속.
- **여기(vectiba.com, 이 레포)에는 매물/경매/딜러승인 같은 실제 제품 데이터·기능을 만들지 말 것.** 문의 폼(`leads` 테이블, 아래 4번)처럼 "관심 등록" 수준 라이트 캡처만.
