# vectiba.com — 현재 상태 (2026-07-13 최신)
...
- `dealer/` 바이어 PC: 에어비앤비식 카드그리드(사이드메뉴X), **밀봉경매**(건수만·낙찰가 표시X), BMW G30 상세(사진 작게+라이트박스), **AI 도착가 계산기**(부가비용 합계 접기/펼치기 + 로드탁송 캐리어/기사 + 화폐선택 + **당일환율** open.er-api.com), [입찰하기]. 에메랄드 헤더 + 화이트/샌드. iframe 높이 자동맞춤(`syncFrameH`, `document.body.scrollHeight`). **ko/en 2개 언어만**(16개 확장 대기).
...
## 5. 남은 일 (다음 세션)
1. **13개 언어 재번역** — 국/영문은 여러 번 바뀌었는데 나머지 14개 언어는 드리프트. 특히 방금 바뀐: `hero_h1`,`hero_def`(한 줄 카피), `nav_buyer`·`buyer_tag`(바이어), 셀러/바이어 섹션 문안. EN 소스와 각 `i18n/<lang>.json` 키 단위 diff로 점검. **번역은 다른 세션이 담당 중** — 국/영문만 확정하고 넘기는 흐름.
2. **딜러 데모 16개 언어** 확장(현재 ko/en).
3. 추가 레이아웃/문구 요청 반영.
